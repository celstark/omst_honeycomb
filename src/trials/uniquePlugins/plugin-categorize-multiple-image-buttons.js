...
const info = {
      name: "categorize-multiple-image-buttons",
      parameters: {
          ...
          stimulus: {
           ...
           /** The second image content to be displayed. */
          stimulus2: {
            type: ParameterType.IMAGE,
            pretty_name: "Stimulus",
            default: undefined,
   ...
   *
   * @author Josh de Leeuw and Craig Stark and Audrey Hempel
   * @see {@link https://www.jspsych.org/plugins/jspsych-categorize-image/ categorize-image plugin documentation on jspsych.org}
   */
  class CategorizeMultipleImageButtonsPlugin {
      constructor(jsPsych) {
          this.jsPsych = jsPsych;
      }
      trial(display_element, trial) {
        var html = '<img id="jspsych-categorize-multiple-image-buttons-stimulus" class="jspsych-categorize-multiple-image-buttons-stimulus" src="' +
                trial.stimulus +
                '"></img> <img id="jspsych-categorize-multiple-image-buttons-stimulus2" class="jspsych-categorize-multiple-image-buttons-stimulus2" src="' +
                trial.stimulus2 +
                '"></img>';
        // hide image after time if the timing parameter is set
        if (trial.stimulus_duration !== null) {
            this.jsPsych.pluginAPI.setTimeout(() => {
                display_element.querySelector("#jspsych-categorize-multiple-image-buttons-stimulus").style.visibility = "hidden";
        ...
        if (Array.isArray(trial.button_html)) {
            if (trial.button_html.length == trial.choices.length) {
                ...
                console.error("Error in categorize-multiple-image-buttons-response plugin. The length of the button_html array does not equal the length of the choices array");
            }
        }
        else {
        ...
        html += '<div id="jspsych-categorize-multiple-image-buttons-response-btngroup">';
        for (var i = 0; i < trial.choices.length; i++) {
            var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
            html +=
                '<div class="jspsych-categorize-multiple-image-buttons-response-button" style="display: inline-block; margin:' +
                    trial.margin_vertical +
                    " " +
                    trial.margin_horizontal +
                    '" id="jspsych-categorize-multiple-image-buttons-response-button-' +
                    i +
                    '" data-choice="' +
                    i +
                    '">' +
        ...
        for (var i = 0; i < trial.choices.length; i++) {
            display_element
                .querySelector("#jspsych-categorize-multiple-image-buttons-response-button-" + i)
        ...
        function after_response(choice) {
            ...
            var btns = document.querySelectorAll(".jspsych-categorize-multiple-image-buttons-response-button button");
        ...
        if (trial.stimulus_duration !== null) {
            this.jsPsych.pluginAPI.setTimeout(() => {
                display_element.querySelector("#jspsych-categorize-multiple-image-buttons-response-stimulus").style.visibility = "hidden";
            }, trial.stimulus_duration);
        }
        ...
        const doFeedback = (correct, timeout) => {
            ...
            if (timeout && !trial.show_feedback_on_timeout) {
                ...
                if (trial.show_stim_with_feedback) {
                    display_element.innerHTML =
                        '<img id="jspsych-categorize-multiple-image-buttons-stimulus" class="jspsych-categorize-multiple-image-buttons-stimulus" src="' +
                        trial.stimulus +
                        '"></img> <img id="jspsych-categorize-multiple-image-buttons-stimulus2" class="jspsych-categorize-multiple-image-buttons-stimulus2" src="' +
                        trial.stimulus2 +
                        '"></img>';
            ...
            display_element.innerHTML += '<div id="jspsych-categorize-multiple-image-buttons-response-btngroup">';
            for (var i = 0; i < trial.choices.length; i++) {
                var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
                display_element.innerHTML +=
                    '<div class="jspsych-categorize-multiple-image-buttons-response-button" style="display: inline-block; margin:' +
            ...
            if (trial.force_correct_button_press &&
                ...
                display_element.querySelector('#jspsych-categorize-multiple-image-buttons-response-button-' + trial.button_answer).addEventListener('click',function(e){
                    end_trial();
                });
            }
            else {
  ...
  CategorizeMultipleImageButtonsPlugin.info = info;

  export { CategorizeMultipleImageButtonsPlugin as jsPsychCategorizeMultipleImageButtons };
